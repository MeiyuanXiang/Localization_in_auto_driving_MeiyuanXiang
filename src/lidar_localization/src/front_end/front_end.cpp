...
namespace lidar_localization
{
    ...
    Eigen::Matrix4f FrontEnd::Update(const CloudData &cloud_data)
    {
        ...
        pcl::removeNaNFromPointCloud(*cloud_data.cloud_ptr, *current_frame_.cloud_data.cloud_ptr, indices); // 剔除NAN错误点

        CloudData::CLOUD_PTR filtered_cloud_ptr(new CloudData::CLOUD());
        cloud_filter_.setInputCloud(current_frame_.cloud_data.cloud_ptr);
        cloud_filter_.filter(*filtered_cloud_ptr); // 降采样
        ...
        // 匹配之后根据距离判断是否需要生成新的关键帧，如果需要，则做相应更新，使用曼哈顿距离
    ...
    void FrontEnd::UpdateNewFrame(const Frame &new_key_frame)
    {
        ...
        // 更新局部地图，实现滑窗就是用一个deque容器把关键帧存储起来，关键帧超过一定数量，就把时间最靠前的关键帧给踢出去。
